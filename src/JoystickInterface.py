...
class JoystickInterface:
    ...
    def build_command(self, state, msg):
        ...
        # Check if requesting a state transition
        # to trotting, or from trotting to resting
        gait_toggle = msg["R1"]
        command.trot_event = (
            gait_toggle == 1 and self.previous_gait_toggle == 0)

        # Check if requesting a state transition
        # to hopping, from trotting or resting
        hop_toggle = msg["x"]
        command.hop_event = (
    ...
    def get_command(self,
        ...
        try:
            '''
            consume UDP message, produce command
            R1: rest to trot, raise trot_event
            x: rest to hop, raise hop_event
            L1: rest to activate, raise activate_event
